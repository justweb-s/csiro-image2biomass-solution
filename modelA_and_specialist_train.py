...
# ============================================================
# 9. Evaluation on validation (evaluation metric)
# ============================================================
...
# ============================================================
# 9. DIVERSITY LOSS (NUMERICALLY STABLE)
# ============================================================
...
class LogCoshLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        # Cast to float32 for safety (avoids immediate fp16 overflow)
        y_pred = y_pred.float()
        y_true = y_true.float()

        diff = y_pred - y_true

        # Stable formula: |x| + softplus(-2|x|) - log(2)
        # This is mathematically equivalent to log(cosh(x)) but avoids exp(x) overflow.
        # log(2) ~= 0.693147
        loss = torch.abs(diff) + F.softplus(-2. * torch.abs(diff)) - 0.693147

        return torch.mean(loss)
 
log_cosh = LogCoshLoss()
 
# Loss weights (ensure float32)
loss_weight_tensor = torch.tensor(
    [0.10, 0.20, 0.20, 0.20, 0.30], dtype=torch.float32, device=device
).view(1, 5)

def biomass_diversity_loss(pred_targets, y_targets, pred_ndvi, y_ndvi, pred_height, y_height):
    # Explicit float32 cast for loss calculations
    pred_targets = pred_targets.float()
    y_targets = y_targets.float()

    # Stable manual computation for multi-targets
    diff = pred_targets - y_targets
    # Same stable formula applied vector-wise
    loss_elements = torch.abs(diff) + F.softplus(-2. * torch.abs(diff)) - 0.693147

    # Weighted mean
    l_targets = (loss_elements * loss_weight_tensor).mean()

    # LogCosh (stable) for auxiliary targets
    l_ndvi   = log_cosh(pred_ndvi, y_ndvi)
    l_height = log_cosh(pred_height, y_height)
...
class ModelEMA:
    def __init__(self, model: nn.Module, decay: float = 0.99):
        # Note: decay 0.99 is more reactive than 0.999, avoids model lag
        self.decay = decay

        # Create a deep copy of the model for EMA
        # For QLoRA, we need to be careful with memory, but deepcopy is the safest method
    ...
    def update(self, model: nn.Module):
        ...
        for k, v in esd.items():
            # In QLoRA/Peft, we only want to update what changes (trainable)
            # or buffers (running stats of batchnorm if present).
            # The int8/nf4 weights of the backbone are frozen in the original model,
            # so msd[k] does not change. We can skip the update for those
            # to save time, or do it anyway (it's a no-op if identical).
            ...
            if v.dtype.is_floating_point:
                ...
                # For integers (or quantized non-float) copy directly
                v.copy_(model_v)
...
# ============================================================
# 11. TRAIN FULL DATA (NO FOLDS, NO VALIDATION)
# ============================================================

def train_full_dataset():
    # Specific folder for the full model
    save_dir = str(MODEL_DIR / "dinov3-swa-full-fit-768px")
    os.makedirs(save_dir, exist_ok=True)

    final_model_path = os.path.join(save_dir, "dinov3_full_last.pth")
    ...
    print(f"\n========== TRAINING FULL DATASET (Img: {IMG_SIZE}) ==========")
    ...
    for epoch in range(1, EPOCHS_STAGE1 + 1):
        ...
        for i, batch in enumerate(pbar):
            ...
            with torch.amp.autocast('cuda'):
                ...
                loss = biomass_diversity_loss(pt, targets, pn, ndvi, ph, height)
        ...
        print(f"S1 Ep {epoch:02d} | Train Loss: {epoch_loss:.4f}")
    ...
    for epoch in range(1, EPOCHS_STAGE2 + 1):
        ...
        scheduler.step()
        epoch_loss = running_loss / len(train_df_full)
        print(f"S2 Ep {epoch:02d} | Train Loss: {epoch_loss:.4f}")
    ...
    # --- FINAL SAVE ---
    # No validation, save final EMA as the definitive model
...
SWA_EPOCHS = 5
SWA_LR = 1e-5
BASE_DIR = str(MODEL_DIR / "dinov3-swa-full-fit-768px")
LAST_MODEL_PATH = os.path.join(BASE_DIR, "dinov3_full_last.pth")
...
# --- CONFIGURATION ---
SAVE_DIR = str(MODEL_DIR / "dinov3-high-specialist-768px")
os.makedirs(SAVE_DIR, exist_ok=True)

# Path to the previously trained SWA model (local)
PRETRAINED_PATH = os.path.join(str(MODEL_DIR / "dinov3-swa-full-fit-768px"), "dinov3_full_SWA.pth")
...
# ============================================================
# 1. Utility functions: asymmetric loss + MixUp
# ============================================================

def asymmetric_loss(pred, target, threshold=80.0, penalty_factor=2.0):
    """
    Applies a higher penalty when the true value is high (> threshold)
...
def train_specialist():
    ...
    # 3. Load previously trained SWA weights
    ...
    for epoch in range(1, EPOCHS_SPECIALIST + 1):
        ...
        for i, batch in enumerate(pbar):
            ...
            with torch.amp.autocast('cuda'):
                ...
                # Asymmetric loss (focus on biomass)
                loss_bio = asymmetric_loss(
                    pred_targets,
                    target_for_loss,
        ...
        print(f"Ep {epoch:02d} | Specialist Loss: {avg_loss:.4f}")
